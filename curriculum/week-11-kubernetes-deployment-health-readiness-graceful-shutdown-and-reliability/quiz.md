...
- **Q1: (B).** A liveness probe that checks the database turns a transient DB blip into a restart storm of healthy processes. Liveness depends on nothing external; only readiness checks dependencies. This is the question that separates operators from authors. Citation: <https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/>.
- **Q2: (B).** Readiness answers "should this pod get traffic?" — so it checks the dependencies the pod needs to serve. A failing readiness pulls the pod from the Service (no restart); when the dependency returns, readiness recovers. Citation: the same probes doc.
- **Q3: (B).** Endpoint removal and `SIGTERM` propagate via different components and asynchronously; `SIGTERM` often arrives first, which is why the readiness-fail-then-wait (or `preStop` sleep) pattern exists. Citation: <https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/#pod-termination>.
- **Q4: (B).** Drain the servers first, close the pool last — otherwise an in-flight handler loses its connection mid-query. Drain the things that use the resource before closing the resource. Citation: <https://pkg.go.dev/net/http#Server.Shutdown>.
- **Q5: (B).** The budget must be under the grace period with a margin, or Kubernetes `SIGKILL`s the process mid-drain — the exact thing graceful shutdown prevents. Citation: the pod-termination doc.
...
- **Q7: (B).** No timeout means each request blocks forever on a slow dependency; goroutines and connections pile up, the pool exhausts, and the service wedges. A `context` deadline converts the hang into a fast failure you can handle. Citation: <https://go.dev/blog/context>.
- **Q8: (B).** Backoff without jitter synchronizes retries into a thundering herd that keeps the dependency down. Full jitter randomizes the wait and spreads the load. Citation: <https://aws.amazon.com/blogs/architecture/exponential-backoff-and-jitter/>.
- **Q9: (B).** An open breaker fails fast (no request sent) for a cooldown, keeping the service responsive and giving the dependency a rest, then half-opens to test recovery. Citation: <https://github.com/sony/gobreaker> and <https://sre.google/sre-book/addressing-cascading-failures/>.
- **Q10: (B).** Shedding returns a fast 503 for excess so the accepted requests stay healthy — bounded concurrency, not an unbounded queue that turns saturation into a latency collapse. Citation: <https://sre.google/sre-book/handling-overload/>.
